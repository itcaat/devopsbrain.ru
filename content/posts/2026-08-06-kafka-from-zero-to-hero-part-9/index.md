...
Kafka часто появляется на собеседованиях не как отдельная технология, а как часть задачи по проектированию распределённой системы. Формально вопрос может быть про Kafka, но на практике проверяют, умеете ли вы думать о порядке, сбоях и последствиях собственных решений.
...
Лучше определить, для какого объекта нужен порядок, и использовать его идентификатор как `Key`. Порядок «вообще для всего» звучит красиво, но часто заканчивается одной перегруженной `Partition`.
