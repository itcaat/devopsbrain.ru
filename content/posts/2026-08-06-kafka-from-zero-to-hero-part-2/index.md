...
Самый же распространённый сценарий когда есть `Key`. В этом случае Kafka вычисляет хэш ключа и уже по хешу сообщение складывается в строго определённый `Partition`. Например, ключом может быть:
...
В нашем примере сообщение всегда попадёт в `Partition 0` по заказу `54321`. Это означает, что все события одного заказа всегда будут храниться в одной партиции, а значит — сохранят правильный порядок.
